...
SHIPPED = [
    ...
    ("Phase 6C.1 Direct Messaging", "May 25, 2026", "Iter 21",
     "User↔Partner and Partner↔Partner direct messages. Opt-in (1b): facilitator+community partners default accepts_new_dms=true, research+vendor default false; if recipient declines, first message creates a `pending` thread requiring Accept/Block. Ombudsman privacy (2c): admins see thread metadata always (participants, counts, timestamps) but message bodies only when a thread is `ombudsman_flagged`. Backend: routers/dm.py with REST surface (open/list/get/send/accept/block/archive/flag/read + /me/dm/preferences + admin metadata-only viewer) and /api/ws/dm/{thread_id} reusing the chat Connection class via a separate dm_registry. Frontend: /dashboard/messages inbox + thread detail with realtime WS, status banners (Pending for recipient / Waiting for sender / Blocked), flag-with-reason modal, archive flow, message bubbles. New components: MessageButton (drop-in on partner profile, facilitator profile, governance board cards, partner directory cards) and a 'To a partner' channel in ShareButton that pushes the share URL into an existing partner thread. PartnerDashboard gains a DmPreferencesCard toggle. Layout dropdown gets Messages + My Bookmarks links. 23/23 backend tests PASS."),
    ("Phase 6C.2 + 6C.3 Ombudsman + Disputes", "May 25, 2026", "Iter 22",
     "Disputes workflow (file → open → under_review → resolved/dismissed) tied to optional transaction_id + thread_id for Step 10 hand-off. Ombudsman queue at /admin/ombudsman aggregating open/under_review disputes plus flagged DM threads. Backend: routers/disputes.py with REST surface for filer (POST /api/disputes, GET /me/disputes filtered by role) and admin/ombudsman (assign/status/resolve). Resolution records financial_credit_usd that Step 10 clawback will consume. Frontend: /dashboard/disputes inbox with type filter chips, /dashboard/disputes/{id} detail (timeline + linked thread/transaction), FileDisputeModal drop-in (wired from thread header + partner profile bottom), /admin/ombudsman queue with 3 stat cards + dispute list + flagged-threads list, /admin/disputes/{id} reuses DisputeDetail with adminMode=true exposing Assign/Move-status/Resolve actions. ProtectedRoute now supports allowOmbudsman prop for users with is_ombudsman=true. Layout user menu gains My Disputes + (for ombudsmen) Ombudsman queue link. 34/34 backend tests PASS."),
]
...
IN_PROGRESS = [
    ("v1.11.0-step10", "Refund/clawback cascade + Partnership agreement v2",
     "Cascade refunds back through credits + payouts, consume disputes.resolution.financial_credit_usd to trigger refunds automatically, and require partners to re-sign agreement v2 on next login."),
]
