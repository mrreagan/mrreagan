...
- **Rollback preview**: new `GET /api/legal/history/{slug}/rollback-preview/{ratification_id}` returns `{target_md, current_md, target_version, current_version, target_ratified_by/at}`. Frontend `HistoryModal` replaces the old `window.prompt` with `RollbackPreviewModal` — side-by-side diff of target-vs-current (jsdiff-powered), reason textarea, empty-state banner + disabled confirm when target=current, confirm fires the existing rollback endpoint.
- **Testing**: `testing_agent` iter 51 — 27/28 backend pytest (1 skip) + 11/11 frontend. Real Resend delivery confirmed for @admin mentions. Regex email-address false-positive caught in code review + fixed with `(?<![\w.])` negative lookbehind (verified: `foo@admin.com` → mentions=[], `Hi @admin` → mentions=['admin'], `Ask @admin about x@counsel.com` → mentions=['admin']).
